...
export class VideosService {
  ...
  async updateProgress(userId: number, updateProgressDto: UpdateProgressDto) {
    ...
    // Considerar completo se >= 98%
    let newProgress = progress >= 98 ? 100 : progress;
    newProgress = Math.min(newProgress, 100);

    return await this.prisma.$transaction(async (tx) => {
      const currentProgress = await tx.videoProgress.findUnique({
      ...
      if (wasCompleted && currentProgress) {
        return {
          ...
          completed: true,
          rewards: { starsEarned: 0, trophyEarned: 0, pointsEarned: 0 },
        };
      }
      ...
      const updatedProgress = await tx.videoProgress.upsert({
        where: { userId_videoId: { userId, videoId } },
        update: { progress: newProgress, stars: newStars, completed: isNowCompleted },
        create: { userId, videoId, progress: newProgress, stars: newStars, completed: isNowCompleted },
      });
      ...
      return {
        ...
        rewards: { starsEarned, trophyEarned },
      };
    });
  }
